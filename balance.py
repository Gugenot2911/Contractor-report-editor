...
class Balance:
    ...
    def __init__(self, site_name):
        ...
        elif isinstance(site_name, (list, tuple)):
            self.site_names = list(site_name)
        else:
            raise ValueError("site_name must be string or list of strings")
    ...
    def refund(self, contractor:str='ОТС')-> pl.dataframe:
        ...
        paths = ['//corp.tele2.ru/NSMRFolders/MR_LOGISTIC/Новосибирск/Возврат/IN 2025 Актуальный новый.xlsx']
        ...
        for file in paths:
            ...
            filtered_df = pl_refund.filter(pl.col('БС').is_in(self.site_names))
    ...
    def sap_os(self, re_filter='') -> pl.dataframe:
        ...
        pl_os = pl_os.filter(
            (pl.col('Субномер') == '0') &
            (pl.col('Сайт').is_in(self.site_names))
        ).drop((['Субномер']))
    ...
    def sap_tmc(self, re_filter='') -> pl.dataframe:
        ...
        pl_tmc = pl.read_excel(path, columns=[3, 4, 8, 9, 10, 20, 21], infer_schema_length=1)
        pl_tmc = pl_tmc.filter((pl.col('Склад').is_in(self.site_names)))
        ...
        pl_tmc = pl_tmc.join(os_file, on='Партия', how='left').fill_null('ТМЦ').drop(['Название основного средства', 'Сайт']) #извлечь номера ОС
...
if __name__ == "__main__":
    print(Balance(site_name=['KZ03']).sap_tmc())
